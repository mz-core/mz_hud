...
const iconMap = {
  heart:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 20.4 4.8 13.7a4.9 4.9 0 0 1 6.9-6.9l.3.3.3-.3a4.9 4.9 0 0 1 6.9 6.9L12 20.4Z"/></svg>',
  shield:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 3 5 6v5c0 5 3.5 8 7 10 3.5-2 7-5 7-10V6l-7-3Z"/></svg>',
  utensils:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 3v7a2 2 0 0 0 2 2h3V3"/><path d="M8 3v18"/><path d="M16 3v9a2 2 0 0 0 2 2h2V3"/><path d="M18 14v7"/></svg>',
  droplet:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 3s-5 6-5 10a5 5 0 0 0 10 0c0-4-5-10-5-10Z"/></svg>',
  ...
  radio:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 11a8 8 0 0 1 16 0"/><path d="M7 11a5 5 0 0 1 10 0"/><path d="M10 11a2 2 0 0 1 4 0"/><rect x="5" y="13" width="14" height="8" rx="2"/><path d="M8 17h.01"/><path d="M11 17h5"/></svg>',
};
...
function voiceLevelFromStatus() {
  const raw = String(
    state.status.voiceMode || state.status.voiceLabel || "",
  ).toLowerCase();
...
function speedometerIcon(name) {
  const icons = {
    ...
    light:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><path d="M9 18h6"/><path d="M10 22h4"/><path d="M12 2a7 7 0 0 0-4 12c.6.5 1 1.2 1 2h6c0-.8.4-1.5 1-2A7 7 0 0 0 12 2Z"/></svg>',
    engine:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><path d="M3 10h3l2-3h5l2 3h3v8H6v-5H3v-3Z"/><path d="M18 13h3v3"/><path d="M10 7V4h4"/></svg>',
  };
  return icons[name] || "";
}
...
function renderSpeedometerDigital(
  speedometer,
  speed,
  rpm,
  fuel,
...
function renderSpeedometerMinimal(
  speedometer,
  speed,
  rpm,
  fuel,
...
function renderSpeedometerRacing(
  speedometer,
  speed,
  rpm,
  fuel,
...
function renderSpeedometerAnalog(
  speedometer,
  speed,
  rpm,
  fuel,
  ...
  const maxSpeed =
    speedometer.unit === "mph" ? (classic ? 120 : 160) : classic ? 200 : 260;
  ...
  const numbers = classic
    ? [0, 40, 80, 120, 160, 200]
        .map((num, i) => {
          const display =
            speedometer.unit === "mph" ? Math.round(num * 0.621371) : num;
...
function renderSpeedometer() {
  ...
  const visible = Boolean(
    speedometer.enabled && state.vehicle.visible && state.speedometerVisible,
  );
  if (!visible) {
  ...
  const style = ["digital", "analog", "minimal", "racing", "classic"].includes(
    speedometer.style,
  )
    ? speedometer.style
    : "digital";
  ...
  dom.speedometer.style.setProperty(
    "--speed-primary",
    speedometer.primary_color,
  );
  dom.speedometer.style.setProperty(
    "--speed-secondary",
    speedometer.secondary_color,
  );
  ...
  const html = {
    digital: renderSpeedometerDigital(
      speedometer,
      speed,
      rpm,
      fuel,
    ...
    minimal: renderSpeedometerMinimal(
      speedometer,
      speed,
      rpm,
      fuel,
    ...
    racing: renderSpeedometerRacing(
      speedometer,
      speed,
      rpm,
      fuel,
    ...
    analog: renderSpeedometerAnalog(
      speedometer,
      speed,
      rpm,
      fuel,
    ...
    classic: renderSpeedometerAnalog(
      speedometer,
      speed,
      rpm,
      fuel,
